...
class ExportMetertoCSV < OpenStudio::Ruleset::ReportingUserScript
  ...
  def run(runner, user_arguments)
	...
	if !meter_names.include? "#{meter_name}"
	  runner.registerError("Meter #{meter_name} is not in sql file.  Please add an AddMeter reporting measure with this variable and run again.")
	end
	...
	if !timeseries.empty?
	  ...
	  headers << "#{meter_name.to_s}"
	  output_timeseries[headers[-1]] = timeseries
	else 
	  runner.registerWarning("Timeseries for #{meter_name} is empty.")
	end
	...
	File.open("./#{meter_name.delete(' ').delete(':')}_#{reporting_frequency.delete(' ')}.csv", 'wb') do |file|
