...
#start the measure
class ExportOutputVariabletoCSV < OpenStudio::Ruleset::ReportingUserScript

  # human readable name
  def name
    return "ExportOutputVariabletoCSV"
  end
	  ...
	  runner.registerError("#{variable_name} is not in sqlFile.  Please add an AddOutputVariable reporting measure with this variable and run again.")
		...
		key_values.each do |key_value|
		  ...
		  if !timeseries.empty?
			...
			headers << "#{key_value.to_s}:#{variable_name.to_s}"
		...
		File.open("./#{variable_name.delete(' ')}_#{reporting_frequency.delete(' ')}.csv", 'wb') do |file|
...
ExportOutputVariabletoCSV.new.registerWithApplication
