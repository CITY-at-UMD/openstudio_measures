...
#start the measure
class MeterFloodPlotBrushable < OpenStudio::Ruleset::ReportingUserScript
  
  #define the name that a user will see, this method may be deprecated as
  #the display name in PAT comes from the name field in measure.xml
  def name
    return " MeterFloodPlot_Brushable"
  end
  ...
  def run(runner, user_arguments)
	...
	simulation_year = year_description.assumedYear()
	if year_description.calendarYear() == nil
	  runner.registerWarning("Calendar year unspecified; using #{simulation_year} to match starting day of week.")
	end
    ...
    sqlFile.availableEnvPeriods.each do |env_pd|	  
	  ...
	  runner.registerInfo("The env_pd is: #{env_pd} the env_type is #{env_type.get.value()}")
	...
	# get desired variable
      ...
      if output_timeseries.is_initialized #checks to see if time_series exists
        output_hourly_plr = []
        output_timeseries_values = output_timeseries.get.values
		output_timeseries_dateTimes = output_timeseries.get.dateTimes
        for i in 0..(output_timeseries_values.size - 1)		
		  temp_date = "#{output_timeseries_dateTimes[i]}"
		  temp_date = temp_date.split("-")
		  
		  if i == (output_timeseries_values.size - 1)
			temp_dateTime = "#{simulation_year+1}-#{temp_date[1]}-#{temp_date[2]}"
		  else
			temp_dateTime = "#{simulation_year}-#{temp_date[1]}-#{temp_date[2]}"
		  end
		  
          temp_array = ['{value:',output_timeseries_values[i],', dateTime:"',temp_dateTime,'", hour:',i%24,', day:',(i/24).round,'},']
          output_hourly_plr << temp_array.join
          value_array <<  output_timeseries_values[i]		  
        end #end of for i in 0..(output_timeseries_values.size - 1)
...
MeterFloodPlotBrushable.new.registerWithApplication